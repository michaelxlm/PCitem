...
describe('parse options', function () {
    ...
    describe('sheet', function () {
        ...
        it('sheetRows n=1', function () {
            ofmt.forEach(function (fmt) {
...
describe('write features', function () {
    ...
    describe('sheet range limits', function () {
        [
            ...
            it(r[0], function () {
                ...
                X.write(wb, wopts);
                wb.Sheets.Sheet1['!ref'] = "A" + X.utils.encode_row(C.r - 5) + ":" + X.utils.encode_cell({
                        r: C.r,
                        c: 0
                    });
                X.write(wb, wopts);
    ...
    it('single worksheet formats', function () {
        ...
        assert.throws(function () {
            X.write(wb, {type: "string", bookType: "csv", sheet: "Sheet3"});
        });
    });
});
...
describe('sylk', function () {
    ...
    describe('input', function () {
        it('codepage', cpavail ? function () {
            ...
            assert.equal(get_cell(X.read(str.replace(/–/, "\x96"), {
                type: "binary",
                codepage: 1252
            }).Sheets.Sheet1, "A1").v, A1);
            if (typeof Buffer !== 'undefined' && !browser) {
                assert.equal(get_cell(X.read(Buffer.from(str), {
                    type: "buffer",
                    codepage: 65001
                }).Sheets.Sheet1, "A1").v, A1);
                assert.equal(get_cell(X.read(Buffer.from(str.replace(/–/, "\x96"), "binary"), {
                    type: "buffer",
                    codepage: 1252
                }).Sheets.Sheet1, "A1").v, A1);
            }
